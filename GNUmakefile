...
debug:
	$(CC) $(CARCHFLAGS) -DDEBUG $(DEBUGCFLAGS) $(INC) $(LDFLAGS) *.c $(LIBS) -o $(binary)
...
ifeq ($(STRIP_BINARY),yes)
	...
	install -m 644 man/man1/iolimitd.1 "$(INSTDIR)"/share/man/man1/
ifeq ($(COMPRESS_MAN),yes)
	rm -f "$(INSTDIR)"/share/man/man1/iolimitd.1.gz
	gzip  "$(INSTDIR)"/share/man/man1/iolimitd.1
endif
...
test: all
	$(CC) -O3 test.c -liolimitd -L. -o test

debugtest: debug
	$(CC) -O0 -ggdb test.c -liolimitd -L. -o test
